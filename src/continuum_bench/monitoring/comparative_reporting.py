...
from .report_profiles import LAYOUTS, CURRENT_LAYOUTS, observed_profiles
...
def generate_comparisons(report):
    ...
    for suite in ('scalability', 'cumulative'):
        for layout in CURRENT_LAYOUTS:
            rows = report.read(report.inputs / f'monitoring/{layout}/{suite}/summary.csv')
            validation_path = report.inputs / f'monitoring/{layout}/{suite}/result-validation.csv'
            validations = report.read(validation_path) if validation_path.is_file() else []
            invalid_reasoners = {
                row.get('reasoner') for row in validations
                if row.get('valid', '').lower() != 'true'
            }
            summaries[layout, suite] = [
                ({**row, 'status': 'invalid_results'}
                 if row.get('reasoner') in invalid_reasoners and outcome(row) == 'completed'
                 else row)
                for row in rows
            ]
    layouts = [layout for layout in CURRENT_LAYOUTS if any(summaries[layout, suite] for suite in ('scalability', 'cumulative'))]
    reasoners = observed_profiles([r for rows in summaries.values() for r in rows])
    # Cross-layout panels include only profiles with at least one population
    # completed in every current layout. A failed profile must not suppress the
    # valid comparisons supplied by the other reasoners.
    comparable_reasoners = [
        reasoner for reasoner in reasoners
        if matched_users([
            [r for r in summaries[layout, 'scalability'] if r.get('reasoner') == reasoner]
            for layout in layouts
        ]) is not None
    ]
    population = matched_users([
        [r for r in summaries[layout, 'scalability'] if r.get('reasoner') == reasoner]
        for layout in layouts for reasoner in comparable_reasoners
    ]) if comparable_reasoners else None
    if population is None:
        report.findings.append('Comparative node/query panels: no population completed across observed layouts and profiles.')
    else:
        excluded = sorted(set(reasoners) - set(comparable_reasoners))
        report.findings.append(f"Matched node/category/query comparisons use synthetic_users={population:g} and profiles={comparable_reasoners}; excluded profiles without valid completed coverage in every layout={excluded}. Worker timings include routing and fan-out.")
        node_rows = []
        for layout in layouts:
            filename = 'node-query-runs.csv' if layout in ('distributed', 'sharded') else 'query-runs.csv'
            raw = report.read(report.inputs / f'monitoring/{layout}/scalability/{filename}')
        ...
        for layout in layouts:
            for reasoner in comparable_reasoners:
        ...
        if categories:
            columns = 2 if len(comparable_reasoners) == 4 else min(3, len(comparable_reasoners))
            rows_count = int(np.ceil(len(comparable_reasoners) / columns))
            fig, axes = plt.subplots(rows_count, columns, squeeze=False,
                                     figsize=(14, 5*rows_count), sharey=True)
            axes = axes.ravel()
            for ax, reasoner in zip(axes, comparable_reasoners):
                for i, layout in enumerate(layouts):
                    ...
                    ax.barh(np.arange(len(categories))+(i-(len(layouts)-1)/2)*(.8/len(layouts)), [statistics.median(v) if v else np.nan for v in vals], .8/len(layouts), label=LAYOUTS[layout])
                ax.set_title(LABELS.get(reasoner,reasoner)); ax.set_xscale('log'); ax.set_xlabel('Total worker query time per repetition (ms)'); ax.grid(axis='x',alpha=.2); ax.legend(fontsize=8)
            for ax in axes[len(comparable_reasoners):]: ax.set_visible(False)
        ...
        for layout in layouts:
            ...
            rows_count = int(np.ceil(len(comparable_reasoners) / columns))
            fig, axes = plt.subplots(rows_count, columns, squeeze=False,
                                     figsize=(14, 5.4*rows_count), sharey=True, sharex=True)
            axes = axes.ravel()
            for ax, reasoner in zip(axes, comparable_reasoners):
                for row in selected:
                ...
                ax.set_title(LABELS.get(reasoner,reasoner)); ax.set_xscale('log'); ax.set_xlabel('Worker query execution (ms)'); ax.grid(alpha=.2)
            ...
            axes[len(comparable_reasoners)-1].legend(handles=[Line2D([], [], marker='o', linestyle='', color=category_colors[c], label=c) for c in cats], loc='upper left', bbox_to_anchor=(1.02, 1), fontsize=8)
        ...
        for layout in layouts:
            ...
            for ax, reasoner in zip(axes, comparable_reasoners):
                ...
                ax.set_title(LABELS.get(reasoner,reasoner));ax.set_xticks(range(len(roles)),roles,rotation=45)
            for ax in axes[len(comparable_reasoners):]: ax.set_visible(False)
            axes[0].set_yticks(range(len(queries)),queries)
            fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap='YlOrRd'), ax=[ax for ax in axes[:len(comparable_reasoners)]], label='Median worker query execution (ms)')
        ...
        fig, axes = plt.subplots(1, len(layouts), figsize=(6*len(layouts), 5), sharey=True, squeeze=False)
        axes = axes.ravel()
        for ax, layout in zip(axes, layouts):
            for i, reasoner in enumerate(comparable_reasoners):
                ...
                ax.bar(np.arange(3)+(i-(len(comparable_reasoners)-1)/2)*(.8/len(comparable_reasoners)), [statistics.median(v) if v else np.nan for v in vals], .8/len(comparable_reasoners), label=LABELS.get(reasoner,reasoner), color=COLORS.get(reasoner))
    ...
    for suite in ('scalability', 'cumulative'):
        ...
        for reasoner in reasoners:
            ...
            if plotted:
                report.save(fig, f'placement-{suite}-{reasoner}', 'Same configured logical progression; completed rows only. Median and min–max; missing later points are not zero.')
            else: plt.close(fig)
