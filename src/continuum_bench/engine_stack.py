...
def _compose(root: Path, *arguments: str) -> None:
    timeout = (
        float(os.environ.get("CONTINUUM_COMPOSE_BUILD_TIMEOUT", "600"))
        if arguments and arguments[0] == "build"
        else float(os.environ.get("CONTINUUM_COMPOSE_TIMEOUT", "120"))
    )
    run_logged(
        [
            "docker",
...
def preflight(root: Path) -> None:
    ...
    checks = runtime_checks() + project_checks(root) + docker_checks()
    for check in checks:
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            error.add_note(
                "Services were retained for diagnostics; logs are under "
                "outputs/runtime/setup/. Stop them after inspection with "
        ...
        if started_here and not keep_running:
            ...
            try:
                ...
                if primary_error is None:
                    error.add_note(
                        "Benchmarks finished, but Docker service cleanup failed."
                    )
                    raise
                primary_error.add_note(f"Docker Compose cleanup also failed: {error}")
                print(
