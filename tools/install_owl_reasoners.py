#!/usr/bin/env python3
"""Install and verify the local native OWL reasoning runtime.
...
def write_environment(classpaths: dict[str, str], konclude: str) -> None:
    lines = [
        ...
        f"export CONTINUUM_OWL_JAVA_HEAP={shlex.quote(os.environ.get('CONTINUUM_OWL_JAVA_HEAP', '2g'))}",
...
def verify_materializers(classpaths: dict[str, str], konclude: str) -> None:
    environment = os.environ.copy()
    for name, classpath in classpaths.items():
        environment[f"CONTINUUM_{name.upper()}_CLASSPATH"] = classpath
    environment["CONTINUUM_OWL_CLASSPATH"] = classpaths["openllet"]
    names = list(JAVA_REASONERS)
    if konclude != "not-installed":
        names.append("konclude")
        environment["CONTINUUM_KONCLUDE_EXECUTABLE"] = konclude.removeprefix("native:")
    for name in names:
        completed = subprocess.run(
            [sys.executable, str(ROOT / "tools/owl/verify_materialization.py"), name],
            env=environment, cwd=ROOT, text=True, capture_output=True, check=False,
        )
        if completed.returncode:
            raise RuntimeError(f"{name} materialization verification failed: {(completed.stdout + completed.stderr)[-4000:]}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--verify-only", action="store_true", help="Verify existing isolated Maven classpaths without downloads")
    parser.add_argument("--java-heap", default=os.environ.get("CONTINUUM_OWL_JAVA_HEAP", "512m" if sys.maxsize <= 2**32 else "2g"),
                        help="Java maximum heap used for verification and saved for workers (e.g. 512m)")
    args = parser.parse_args(argv)
    if not re.fullmatch(r"[1-9][0-9]*[kKmMgG]?", args.java_heap):
        parser.error("--java-heap must be a Java heap size such as 512m or 2g")
    os.environ["CONTINUUM_OWL_JAVA_HEAP"] = args.java_heap
    try:
        classpaths = (
            {name: path.read_text(encoding="utf-8").strip() for name, path in CLASSPATH_FILES.items()}
            if args.verify_only else install_java_reasoners()
        )
        ...
        verify_materializers(classpaths, konclude)
